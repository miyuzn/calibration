...
def batch_process_csv_files():
    ...
    for root, _, files in os.walk(f"./{exp}/"):
        for file in files:
            # if file.endswith(".csv") and file != os.path.basename(recur_results_csv):
            if file.endswith(".csv") and not re.search(r"_([rR]|[fF])\.csv$", file):
                # Handle file paths
                sensor_input_path = os.path.join(root, file)
                output_path = os.path.splitext(sensor_input_path)[0] + "_r.csv"  # Modify output file path with '_output.csv' suffix
